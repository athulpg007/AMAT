...
s1 = mlab.mesh(x, y, z, color=(0.9,0,0.1))
s2 = mlab.mesh(x1, y1, z1, color=(0.9,0,0.1), opacity=0.3)
...
for psi in np.linspace(0, 2 * np.pi, 31):
	probe = Approach("MARS",
					...
					rp=(3389 + 20) * 1e3, psi=psi,
					is_entrySystem=True, h_EI=120e3)

	theta_star_arr_probe1 = np.linspace(-1.8, probe.theta_star_entry, 101)
	pos_vec_bi_arr_probe1 = probe.pos_vec_bi(theta_star_arr_probe1) / 3389e3

	x_arr_probe = pos_vec_bi_arr_probe1[0][:]
	y_arr_probe = pos_vec_bi_arr_probe1[1][:]
	z_arr_probe = pos_vec_bi_arr_probe1[2][:]

	mlab.plot3d(x_arr_probe, y_arr_probe, z_arr_probe, color=(0, 1, 0), line_width=3, tube_radius=None)
