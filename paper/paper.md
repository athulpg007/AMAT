...
AMAT can be used to compute aerocapture trajectories as well as key design parameters such as deceleration and stagnation-point heat rate using just a few lines of code. Figure 1 shows the undershoot and overshoot trajectories for a lift modulation aerocapture vehicle at Venus, and the stagnation-point heat rates computed using AMAT (based on the vehicle and entry parameters used by [@Craig2005]).

![](https://i.imgur.com/ZzRzY6W.png)
**Figure 1.** Venus aerocapture trajectory (undershoot and overshoot limits) and stagnation-point heat rates computed using AMAT. 
